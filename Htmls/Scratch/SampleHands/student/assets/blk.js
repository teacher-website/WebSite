...
count:[
  ...
  SETV('拇指距離',ABS(OP(HX(5),'-',HX(6))),{hl:1,note:'拇指尖離食指掌根有多遠（橫向）'}),
  SETV('拇指門檻',OP(V('手寬'),'×',V('拇指比例')),{note:'拇指比例預設 0.45'}),
  SETV('拇指餘裕',OP(V('手寬'),'×',V('餘裕比例'))),
  {k:'ce',c:'control',t:['如果',B(V('拇指'),'=',N(1)),'那麼'],hl:1,
   note:'拇指是往「旁邊」伸的，y 幾乎不變，\n所以改成量橫向距離：\n　距離 > 手寬 × 0.45 ＝ 伸直\n一樣用餘裕做遲滯',body:[
    {k:'c',c:'control',t:['如果',B(V('拇指距離'),'<',OP(V('拇指門檻'),'-',V('拇指餘裕'))),'那麼'],body:[SETV('拇指',N(0))]}
  ],body2:[
    {k:'c',c:'control',t:['如果',B(V('拇指距離'),'>',OP(V('拇指門檻'),'+',V('拇指餘裕'))),'那麼'],body:[SETV('拇指',N(1))]}
  ]},
...
main:[
  ...
  SETV('拇指比例',N(0.45)),SETV('掉格容忍',N(0.4)),
  {k:'forever',c:'control',t:['重複無限次'],body:[
